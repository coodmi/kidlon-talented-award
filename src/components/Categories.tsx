...
import { Music, Palette, FlaskConical, Dumbbell, BookOpen, Code2, Mic2, Film, Leaf, Calculator, Drama, Lightbulb } from "lucide-react";
...
export default function Categories() {
  return (
    <section id="categories" className="py-16 sm:py-20 lg:py-24 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 sm:mb-14">
          ...
          <h2 className="section-title text-white mb-4">
            12 Categories of <span className="gradient-text">Excellence</span>
          </h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
          {categories.map((cat) => (
            <div
              key={cat.title}
              className={`group relative p-4 sm:p-5 rounded-2xl ${cat.bg} border ${cat.border} card-hover cursor-pointer overflow-hidden`}
            >
              ...
              <div className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-br ${cat.color} flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                <cat.icon className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
              <h3 className="text-white font-bold text-xs sm:text-sm md:text-base mb-1 leading-tight">{cat.title}</h3>
              <p className="text-gray-400 text-xs leading-relaxed hidden sm:block">{cat.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center mt-10 sm:mt-12">
          ...
          <a
            href="#nominate"
            className="inline-flex items-center gap-2 px-6 sm:px-8 py-3 sm:py-4 rounded-xl bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-bold text-sm sm:text-base hover:from-yellow-300 hover:to-yellow-400 transition-all duration-300 glow-gold hover:scale-105"
          >
